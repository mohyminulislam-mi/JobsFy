...
import { ArrowLeft, Send, Sparkles, User, RefreshCw, MoreVertical, Paperclip } from 'lucide-react';

export default function AIChatPage() {
  const [messages, setMessages] = useState([
    { role: 'ai', content: "Hi there! I'm Jobfy AI. I can help you find jobs, optimize your resume, prepare for interviews, or answer any career-related questions. How can I assist you today?" }
  ]);
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsTyping(false);
      setMessages(prev => [...prev, { role: 'ai', content: "That's a great question! Based on your profile, I recommend highlighting your React expertise and focusing on remote-first companies. Would you like me to find some open roles matching that criteria?" }]);
    }, 1500);
  };
  ...
  const handleSuggestion = (text: string) => {
    ...
    setTimeout(() => {
      setIsTyping(false);
      setMessages(prev => [...prev, { role: 'ai', content: "Here's what I found for you..." }]);
    }, 1500);
  };
      ...
      <div className="max-w-4xl mx-auto w-full px-4 sm:px-6 lg:px-8 flex flex-col h-full gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-soft border border-gray-100 p-4 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-4">
            <Link href="/jobs" className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-xl transition-colors">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-bold text-gray-900 leading-tight flex items-center gap-2">Jobfy AI Assistant</h2>
                <p className="text-xs text-primary-600 font-medium tracking-wide">Always learning, always ready</p>
              </div>
            </div>
          </div>
          <div className="flex gap-2">
            <button className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-xl transition-colors" title="Start over">
        ...
        <div className="bg-white rounded-2xl shadow-soft border border-gray-100 flex-grow flex flex-col overflow-hidden relative">
          ...
          <div className="flex-grow p-6 overflow-y-auto flex flex-col gap-6 custom-scrollbar scrollbar-hide">
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-gray-200' : 'bg-primary-100 text-primary-600'}`}>
                  {msg.role === 'user' ? <User className="w-4 h-4 text-gray-600" /> : <Sparkles className="w-4 h-4" />}
                </div>
                <div className={`max-w-[80%] rounded-2xl px-5 py-3.5 text-sm md:text-base leading-relaxed ${
                  msg.role === 'user' 
              ...
              <div className="mt-auto grid md:grid-cols-2 gap-3 mb-4">
                {[
                  ...
                  >
                    ...
                    <ArrowLeft className="w-4 h-4 rotate-135 text-primary-400 opacity-0 group-hover:opacity-100 group-hover:-translate-y-1 group-hover:translate-x-1 transition-all" style={{ transform: 'rotate(135deg)' }} />
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="p-4 border-t border-gray-100 bg-white">
            <form onSubmit={handleSend} className="relative">
              <button type="button" className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-primary-600 transition-colors p-2">
            ...
            <p className="text-center text-xs text-gray-400 mt-3 font-medium">Jobfy AI can make mistakes. Consider verifying important information.</p>
          </div>
